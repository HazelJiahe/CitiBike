...
simulate the system as well as record the gradient (i.e. charging_amount: the amount of charge a station can potentially provide given it is electrified) 
'''

'''
In the future Citi Bike may replace all regular bikes with electrified bikes in NYC;
Citi Bike may choose a certain percentage of stations to be charging stations to optimize the system performance;
E-bike speed in NYC is 10mph;
Depletion rate is 40% per hour and it takes around 3 hours to fully charge a depleted e-bike;
If there is no e-bike available in the origin station or the destination station is full, then the trip is canceled (demand lost); 
Otherwise, an e-bike with highest state of charge (SOC) gets picked up;
A depleted e-bike can still work as a regular bike with a lower speed (around 4.4 mph);
'''
...
class Bike(object):
    ...
    def __init__(self, id, stationid,isebike,charging = False,trip_times=0):
        ...
        self.lastchargetime = time(2017,7,1,hour= 1)
...
class GlobalClock(object):
    ...
    def __init__(self, start_time,end_time, initial_stations,random_seed = 1):
        ...
        self.week_out_of_battery_duration = {}
        ...
        self.week_out_of_battery_duration[0] = 0
        ...
        self.out_of_battery_duration = 0
        self.week_num_etrip={}
        ...
        while(self.t <= self.end_time):
            ...
            if (self.t- self.start_time).days % 7==0:
                ...
                if self.weeks[self.week]==False:
                    ...
                    self.week_out_of_battery_duration[self.week+1] = self.out_of_battery_duration - sum(self.week_out_of_battery_duration.values())
    ...
    def clear_charging_info(self,):
        for sta in self.stations.keys():
            self.stations[sta].charging_amount = 0
...
def trip_generate(globalclock,next_event):
    # If there is no e-bike available in the origin station or the destination station is full, 
    # then the trip is canceled (demand lost); Otherwise, the e-bike with highest state of charge will be picked up for the trip
    ...
    for j in gc.stations[stationid].ebike.keys():
        ...
        gc.stations[stationid].charging_amount += min(100,new_SOC) - gc.bikes[j].SOC
        if gc.bikes[j].charging == True: #bike j is in e-station 
...
def generate_ebike_trip(globalclock,stationid,dest,ebikeid,pickupebike=True):
    ...
    if remaining_battery>0:
        ...
        gc.out_of_battery_duration += (-remaining_battery)/energy*seconds*slow_ratio
        end_SOC=0
